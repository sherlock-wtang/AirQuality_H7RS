/**
  ...
  * @brief   Template for HAL time base based on the peripheral hardware TIM1.
  *
  *          This file override the native HAL time base functions (defined as weak)
  *          the TIM time base:
  *           + Initializes the TIM1 peripheral to generate a Period elapsed Event each 1ms
  ...
  */
  ...
  HAL_TIM_RegisterCallback(&htim1, HAL_TIM_PERIOD_ELAPSED_CB_ID, TimeBase_TIM_PeriodElapsedCallback);
    ...
    if (Status == HAL_OK)
    {
      ...
      {
        /* Configure the TIM1 global Interrupt priority */
...
/**
  ...
  * HAL_TIM1_IRQHandler(). It makes a direct call to HAL_IncTick() to increment
